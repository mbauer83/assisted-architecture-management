...
- [x] **WU-D2 — Connection selection, matrix bridging, provenance fields**
  ...
  - Changes: derived relationships between included entities join the display set under
    ...
    wall-clock time budget (PLAN §5.5): derivation stopping early on
    `derivation_time_budget_seconds` returns its partial result flagged `truncated`, always
    warned via `ViewpointProjection.warnings`; only the `derivation_max_relationships` hard
    ceiling raises `DerivationLimitError` (typed error, whole execution aborts, no partial
    result).**
...
- [x] **WU-H5 — Failure-mode & accessibility coverage (PLAN §10 failure contract)**
...
- [x] **WU-I1 — Modeling docs**
  ...
  - Deps: E3a, E3b, G2.
- [x] **WU-I2 — Reference docs & regenerated surfaces**
...
- [x] **WU-I3 — Conformance page & README**
...
- 2026-07-14 — Post-H4 hardening (cont.) — Backend restarted; live-verified all prior fixes. Collapsed multiple derived-relationship types found between the same entity pair down to the single best one (`_record_if_incident`'s dedup key changed from `(source, target, type)` to `(min(source,target), max(source,target))`, keeping the most-certain/shortest-hop/deterministic-tiebreak result) — user-confirmed via AskUserQuestion. Built the witness-chain-in-sidebar feature for derived connections on the diagram view: shared `useWitnessChain.ts` composable (`WitnessChainReadAccess`/`WitnessChainDisplay`, path reconstruction from the unordered `via_connection_ids` set) used by both `WitnessChainPopover.vue` and a new `.det-derived` section in `DiagramEntitySidebar.vue`; `markDerivedConnections` tags derived SVG edges with `data-certainty` as a click hook. E2e-verified against a rebuilt small deterministic fixture — the first attempt mirrored `motivation-layer-support`'s full-domain shape (3159 connections) and crashed PlantUML, caught live by the user ("only one connection per pair, remember?"); rebuilt to mirror `element-dependents`'s single-anchor shape (58 connections). Added the traversal/include-potential/max-hops authoring controls to `ViewpointQueryTab.vue`'s neighbor-inclusion and connections sections, which WU-H1 had never surfaced despite the frontend criteria model already carrying those fields since WU-H3 — silently forcing every GUI-authored viewpoint to `direct`-only traversal (user: "without traversal: derived, the viewpoint is useless, no? So are many others"); e2e round-trip test added (`.fill()` on the hops input needed an explicit `.blur()` — Playwright's `fill()` dispatches `input`, not `change`, which the component listens on). User caught a wrong claim (asserted no PDR rule composes a dependency relationship with a structural one) by citing Example B-12/PDR12 by name; re-reading the spec confirmed PDR5 covers exactly this pattern, and a direct diagnostic script proved `NOD↔CLI-tool` composes correctly — the original claim was a bookkeeping error in which of two real edges the BFS treats as first/second per anchor direction, not a spec gap. Investigated why `motivation-technology-support` still showed zero technology entities: first fixed a real, separate bug (`include_potential` was never set on its neighbor inclusion, unlike its sibling `motivation-layer-support`), then proved via full-population `artifact_query_viewpoint execute` (no truncation, unlimited-time control run) that the pairing is nonetheless structurally empty in this repo — the only bridge found from any technology node toward business/motivation passes through an Association relationship (`architecture-backend`—`architecture-management-system`), and Appendix B correctly excludes Association from further derivation composition; not a bug, recorded as such in the definition's own description. Built and verified a working cross-domain derivation example instead, `business-technology-support` (business-domain primary, technology-domain derived neighbor, skipping the application layer) — 41 entities / 75 connections, no truncation; first attempt had primary/neighbor reversed ("Technology as supported by / supporting Business" — user caught the backwards causality), corrected to business-as-subject naming matching the `motivation-technology-support` convention. 4913/4913 pytest (0 unrelated failures; only the tracked LoC backlog remains) in 104s properly parallelized (confirmed after a `head`-piping artifact briefly made a rerun look slow), ruff/zuban clean; frontend typecheck/lint/Vitest (790/790) clean; 64/64 e2e.
- 2026-07-14 — WU-H5 — Audited H0–H4 surfaces against PLAN §10's failure/accessibility contract before writing anything, to separate genuine gaps from already-covered ground. Five real gaps found and closed: (1) a failed definition save (network error, not a validation 400) had no retry affordance — `ViewpointsManagementView.vue` conflated it with the unrelated list-load error; split into a dedicated `saveError` ref with its own retry button, reusing `ViewpointExecutionError.vue`'s established pattern (edits were already preserved — the catch handler never touched the draft, only the missing UI needed building). (2) the scope-picker's zero-match "No types match…" hint already existed in the markup but had no test. (3) the stale-derived-connection banner (ephemeral layered-exploration flow) was one all-or-nothing "Dismiss" for an opaque count; replaced with per-finding rows (`staleAcceptedFindings`/`clearDecision` in `derivedCandidateReview.ts`, snapshotting the previous render's connections/entities before the re-run overwrites them) each showing source→target→type with independent Re-review (reopens the existing witness-chain popover) and Remove actions. Deliberately did NOT build the PLAN §5.7 persisted-diagram accept/re-review/remove flow this acceptance bullet also describes — WU-H3 explicitly scoped that "generate+persist a diagram from a strategy" machinery out (user-confirmed, `view_derivations.py`/`refresh.py`/`path_staleness.py` remain unwired), and no WU anywhere builds that GUI; read the criterion against what actually exists rather than reopen a locked scope decision unprompted. (4) `CriteriaTreeBuilder.vue`/`ConditionRow.vue` had zero `aria-label`s on any icon-only button or unlabeled select — added labels + `aria-pressed` on both NOT-toggles, consistent with the existing chip pattern in `ViewpointScopeTab.vue`; native `<button>`/`<select>` elements already guaranteed Tab-order/Enter-Space operability, so no custom key handling was needed, just names for assistive tech. (5) only the generic network-abort fallback text was e2e-triggered for execution failures; added one test per typed code (`execution-timeout`/`derivation-limit`/`binding-cardinality-violation`/`parameter-type-mismatch`) mocking the real `{code,path,message}` 400 shape and asserting each renders its own distinct title. One test-authoring mistake caught by a full-suite (not isolated) run: `getByRole('button', {name: 'Remove'})` substring-matched the new "Remove condition" button on the same page (the "By criteria" root-selection mode mounts a criteria builder) — fixed with `exact: true`, the same class of bug the WU-H4 note already recorded once. Also confirmed the pytest timing scare from the prior entry was a red herring: parallelization was never broken (1445% CPU observed), a `head`-piped rerun just got SIGPIPE-truncated at 20s. 4913/4913 pytest (untouched this WU — 0 unrelated failures beyond the tracked LoC backlog, confirmed clean before starting), ruff/zuban clean; frontend typecheck/lint/Vitest (791/791) clean; 72/72 e2e (one unrelated pre-existing flake in `viewpoint-diagram-view.spec.ts` reproduced only under full-suite load, confirmed passing in isolation, not a regression). `rg -i "WU-|phase [a-i]\b|companion plan"` clean on every file this WU touched.
- 2026-07-14 — WU-I1 — Extended `viewpoints.md` with parameters/bindings/derived-attributes/traversal/scope-only sections, and added new `impact-analysis.md` (certain-vs-potential, the role/strength composition table, the Association-composition-boundary finding from the post-H4 investigation written up as documented behavior rather than left as tribal knowledge, impact workflows, materializing a candidate); linked both into `index.md`'s page table and each other's footer. Every worked example is a real thing: `element-dependents`/`business-technology-support`/`goal-realization` are live shipped/created definitions; the bindings snippet is the exact YAML from an `artifact_viewpoint` dry-run that validated clean (iterated twice — a scalar `entity[requirement]` result type rejects `in`, needed the plural `entities[requirement]` list type). One stray decision-ID reference (`D9's scope fallback`) caught by the required `rg` sweep and reworded before ticking.
