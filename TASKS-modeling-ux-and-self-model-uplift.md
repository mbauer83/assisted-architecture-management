...
| A2 | Shared `PreviewViewport.vue`; adopt in Create/EditDiagramView | — | review | code done; needs live browser check (no Chrome in this sandbox) |
...
| A1.2 | Frontend: single picker `doSearch` path + scroll lazy-load + tests | A1.1 | review | code done; needs live browser check (no Chrome in this sandbox), same as A2 |
| A4.1 | `mapElements` viewer-extension contract (`Map<id, Element[]>`) + graphviz default extraction | — | done | contract unit tests |
| A4.2 | Sequence: `display_alias` participants + viewer extension (DOM-order messages) | A4.1 | review | code+tests done; needs live Playwright check (no Chrome in this sandbox), same as A2/A1.2 |
| A4.3 | Activity: sentinel id-links + viewer extension (order fallback) | A4.1 | review | code+tests done; needs live Playwright check (no Chrome in this sandbox), same as A2/A1.2/A4.2 |
...
| B2b | Wizard domain stages: create / find / connect + commit flow | B2a, A1.2 | todo | commit semantics = D-6 |
| B2c | Elicitation layers (questionnaire, impact spine, capability anchor) + ranking | B2b | todo | content-driven step definitions |
...
| D-6 | Wizard commit semantics: per-artifact verified commits + undo (recommended) vs. staged-bulk atomic REST commits | **open** — ask before B2b |
