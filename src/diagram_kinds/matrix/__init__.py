...
class _MatrixRenderer:
    def render_body(
        ...
        del name, entities, connections, diagram_type, repo_root
...
class _MatrixDiagramKind(DiagramKindBase):
    def __init__(self, config: dict[str, Any]) -> None:
        self._config = config

    @property
    def name(self) -> DiagramKindName:
        return DiagramKindName(str(self._config["name"]))
...
module: DiagramKindModule = _MatrixDiagramKind(_load_config(Path(__file__).parent))
