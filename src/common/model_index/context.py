...
def rebuild_entity_context_projection(index: "ModelIndex") -> None:
    rows = [
        row
...
def entity_context_rows_for_connection(index: "ModelIndex", rec: ConnectionRecord) -> list[tuple[str, ...]]:
...
def rebuild_entity_context_for(index: "ModelIndex", entity_id: str) -> None:
...
def recompute_entity_context_stats(index: "ModelIndex", entity_id: str) -> None:
...
def apply_entity_file_change(index: "ModelIndex", path: Path, *, bump_generation: bool = True) -> None:
    index._ensure_loaded()
    with index._lock:
...
def apply_outgoing_file_change(index: "ModelIndex", path: Path, *, bump_generation: bool = True) -> None:
    index._ensure_loaded()
    with index._lock:
...
def apply_diagram_file_change(index: "ModelIndex", path: Path, *, bump_generation: bool = True) -> None:
    index._ensure_loaded()
    with index._lock:
...
def read_entity_context(index: "ModelIndex", entity_id: str) -> EntityContextReadModel | None:
    index._ensure_loaded()
