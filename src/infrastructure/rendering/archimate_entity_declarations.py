...
def entity_declaration(entity: EntityRecord, alias: str, registry: Any, junction_types: frozenset[str]) -> str:
    if entity.artifact_type in junction_types:
        return f'circle " " as {alias}'
    label, stereotype = entity_label_and_stereotype(entity, registry)
    if stereotype and entity_has_sprite(entity, registry):
...
def entity_nest_declaration(entity: EntityRecord, alias: str, registry: Any, junction_types: frozenset[str]) -> str:
    if entity.artifact_type in junction_types:
        return f'circle " " as {alias}'
    label, stereotype = entity_label_and_stereotype(entity, registry)
    if stereotype and entity_has_sprite(entity, registry):
...
def entity_label_and_stereotype(entity: EntityRecord, registry: Any) -> tuple[str, str | None]:
