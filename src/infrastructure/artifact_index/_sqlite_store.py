...
_INS_ENTITY = "INSERT INTO entities (artifact_id,artifact_type,name,version,status,domain,subdomain,path,scope,keywords_json,extra_json,content_text,display_blocks_json,display_label,display_alias) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
_INS_CONNECTION = "INSERT INTO connections (artifact_id,source,target,conn_type,version,status,path,scope,extra_json,content_text,associated_entities_json,src_cardinality,tgt_cardinality) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
_INS_DIAGRAM = "INSERT INTO diagrams (artifact_id,artifact_type,name,diagram_type,version,status,path,scope,extra_json) VALUES (?,?,?,?,?,?,?,?,?)"
_INS_DOCUMENT = "INSERT INTO documents (artifact_id,doc_type,title,status,path,scope,keywords_json,sections_json,content_text,extra_json) VALUES (?,?,?,?,?,?,?,?,?,?)"
_INS_EDGE = "INSERT INTO entity_context_edges (entity_id,connection_id,direction_bucket,other_entity_id,conn_type,connection_status,connection_version,source_id,target_id,source_name,target_name,source_artifact_type,target_artifact_type,source_domain,target_domain,source_scope,target_scope,path,content_text,associated_entities_json,src_cardinality,tgt_cardinality) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
_INS_EFTS = "INSERT INTO entities_fts (artifact_id,name,artifact_type,domain,subdomain,keywords,content_text,display_label) VALUES (?,?,?,?,?,?,?,?)"
...
class _SqliteStore:
    ...
    @property
    def conn(self) -> sqlite3.Connection: return self._conn
    @property
    def fts_enabled(self) -> bool: return self._fts_enabled
    ...
    def upsert_entity(self, rec: EntityRecord) -> None:
        ...
        with self._conn:
            ...
            if self._fts_enabled:
                ...
                self._conn.execute(_INS_EFTS, (rec.artifact_id, rec.name, rec.artifact_type, rec.domain, rec.subdomain, " ".join(rec.keywords), rec.content_text, rec.display_label))
    ...
    def upsert_connection(self, rec: ConnectionRecord) -> None:
        ...
        with self._conn:
            ...
            if self._fts_enabled:
                ...
                self._conn.execute(_INS_CFTS, (rec.artifact_id, rec.source, rec.target, rec.conn_type, rec.content_text))
    ...
    def upsert_document(self, rec: DocumentRecord) -> None:
        ...
        with self._conn:
            ...
            if self._fts_enabled:
                ...
                self._conn.execute(_INS_DOCFTS, (rec.artifact_id, rec.title, rec.doc_type, " ".join(rec.keywords), rec.content_text))
    ...
    def rebuild(self) -> None:
        with self._conn:
            for t in ("entities", "connections", "diagrams", "documents", "entity_context_edges", "entity_context_stats"):
            ...
            if self._fts_enabled:
                self._conn.executemany(_INS_EFTS, [(r.artifact_id, r.name, r.artifact_type, r.domain, r.subdomain, " ".join(r.keywords), r.content_text, r.display_label) for r in self._mem.entities.values()])
                self._conn.executemany(_INS_CFTS, [(r.artifact_id, r.source, r.target, r.conn_type, r.content_text) for r in self._mem.connections.values()])
                self._conn.executemany(_INS_DFTS, [(r.artifact_id, r.name, r.diagram_type, r.artifact_type) for r in self._mem.diagrams.values()])
                self._conn.executemany(_INS_DOCFTS, [(r.artifact_id, r.title, r.doc_type, " ".join(r.keywords), r.content_text) for r in self._mem.documents.values()])
    ...
    def rebuild_context_projection(self) -> None:
        ...
        with self._conn:
            ...
            self._conn.execute(
                ...
                "SELECT entity_id, SUM(direction_bucket='inbound'), SUM(direction_bucket='outbound'), SUM(direction_bucket='symmetric') "
    ...
    def rebuild_context_for(self, entity_id: str) -> None:
        with self._conn:
            ...
            rows = [row for rec in self._mem.connections.values() for row in self._context_rows(rec) if row[0] == entity_id]
            if rows:
                self._conn.executemany(_INS_EDGE, rows)
    ...
    def _entity_row(self, r: EntityRecord) -> tuple[str, ...]:
        return (r.artifact_id, r.artifact_type, r.name, r.version, r.status, r.domain, r.subdomain, str(r.path), self._scope(r.path), json.dumps(list(r.keywords)), json.dumps(r.extra, sort_keys=True), r.content_text, json.dumps(r.display_blocks, sort_keys=True), r.display_label, r.display_alias)

    def _connection_row(self, r: ConnectionRecord) -> tuple[str, ...]:
        return (r.artifact_id, r.source, r.target, r.conn_type, r.version, r.status, str(r.path), self._scope(r.path), json.dumps(r.extra, sort_keys=True), r.content_text, json.dumps(list(r.associated_entities)), r.src_cardinality, r.tgt_cardinality)

    def _diagram_row(self, r: DiagramRecord) -> tuple[str, ...]:
        return (r.artifact_id, r.artifact_type, r.name, r.diagram_type, r.version, r.status, str(r.path), self._scope(r.path), json.dumps(r.extra, sort_keys=True))

    def _document_row(self, r: DocumentRecord) -> tuple[str, ...]:
        return (r.artifact_id, r.doc_type, r.title, r.status, str(r.path), self._scope(r.path), json.dumps(list(r.keywords)), json.dumps(list(r.sections)), r.content_text, json.dumps(r.extra, sort_keys=True))
    ...
    def _context_rows(self, rec: ConnectionRecord) -> list[tuple[str, ...]]:
        ...
        shared: tuple[str, ...] = (
            rec.conn_type, rec.status, rec.version, rec.source, rec.target,
            src.name if src and src.name else rec.source,
            tgt.name if tgt and tgt.name else rec.target,
            src.artifact_type if src else "unknown", tgt.artifact_type if tgt else "unknown",
            src.domain if src else "unknown", tgt.domain if tgt else "unknown",
            self._scope(src.path) if src else "unknown", self._scope(tgt.path) if tgt else "unknown",
            str(rec.path), rec.content_text, json.dumps(list(rec.associated_entities)),
            rec.src_cardinality, rec.tgt_cardinality,
        )
        if rec.conn_type in SYMMETRIC_CONNECTIONS:
            ...
            return rows
        return [(rec.source, rec.artifact_id, "outbound", rec.target, *shared),
