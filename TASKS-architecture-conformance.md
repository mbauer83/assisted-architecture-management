...
| WU-05 | C | Migrate app/infra consumers; delete free-function catalogs; remove globals; finish snapshot removal | WU-04 | ▶ NEXT |
| WU-06 | C | CLI + MCP composition roots inject catalogs; drop all `domain→infra` baseline entries | WU-05 | ☐ todo |
| WU-07 | D | Remove `domain→config` + `domain→ontologies`; classify plugin packages in arch test | WU-06 | ☐ todo |
