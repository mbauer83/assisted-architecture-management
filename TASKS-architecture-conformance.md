...
| WU-19 | K | SVG sprite converter port — decouple `archimate_next/_loader.py` from infra | WU-12 | ✅ done |
| WU-20 | K | Complete injection — remove all lazy `app_bootstrap` fallbacks; empty baseline | WU-18, WU-19 | ✅ done |
| WU-14 | I | Glossary doc + responsibility-driven renames | WU-08, WU-09, WU-12, WU-20 | ✅ done |
| WU-15 | J | Self-model (Model Registry/Verifier) + README + stale docstrings | WU-14 | ✅ done |
| WU-16 | J | Verifier conformance check for self-model `Module:` source paths | WU-15 | ✅ done |
...
- 2026-06-08 · WU-19 · arch-conformance@389b288 · gates: all pass (1720 pytest, 1 skipped, ruff clean, zuban clean, arch test green) · _ArchiMateNextModule accepts svg_converter: Callable[[str], str] | None in __init__; sprite_for uses injected converter (None → returns None); load_archimate_next_module gets svg_converter kwarg; app_bootstrap imports browser_markup_to_plantuml_svg from infra and passes it at construction time; archimate_next module singleton from __init__.py no longer used in bootstrap (builds its own wired instance); baseline entry (ontologies/archimate_next/_loader.py→infra._svg_sprite_convert) removed. Baseline: 7→6 entries. PLAN §6 Phase K3 checkbox ticked.
- 2026-06-09 · WU-20 · arch-conformance@b980fd5 · gates: all pass (1720 pytest, 1 skipped, ruff clean, zuban clean, arch test green) · Removed all 6 remaining lazy app→infra fallbacks: artifact_parsing.py + entity_type_predicates.py + modeling/artifact_write.py + modeling/matrix_builder.py + verification/artifact_verifier.py (catalog default + 4 adapter defaults). New src/application/verification/_verifier_stdlib_adapters.py houses stdlib-only adapter impls (ThreadPoolVerifierScheduler, FilesystemInventoryAdapter, DefaultIncrementalStateAdapter, _NullPumlSyntax); infra/verification/adapters.py re-exports them and retains only DefaultPumlSyntaxAdapter. ArtifactVerifier raises RuntimeError if _runtime_catalogs accessed without catalogs=. artifact_parsing.py + entity_type_predicates.py gained explicit catalog/ontology/module_catalog params; ~20 infra callers updated to build catalogs lazily; ArtifactIndex gained _get_domain_names(). architecture_baseline.json is now []. PLAN §6 Phase K4/K5 checkboxes ticked; §8 DoD "baseline empty" + "dependency matrix" checkboxes ticked.
- 2026-06-09 · WU-14 · arch-conformance@0739b3d · gates: all pass (1720 pytest, 1 skipped, ruff clean, zuban clean, arch test green) · Added docs/architecture/glossary.md with 7 terms: Module Catalog, Artifact Index, Artifact Repository, Application Composition Root, Runtime Host, Verification Policy, Verification Executor. No file renames: diagram_type_registry done in WU-08; strategy_registry.py has 11 importers and the PLAN's own "avoid cosmetic churn" guidance applies. PLAN §6 Phase I1/I2/I3 checkboxes ticked; §8 DoD "Glossary published" ticked.
- 2026-06-09 · WU-15 · arch-conformance@b5dd86b · gates: all pass (1720 pytest, 1 skipped, ruff clean, zuban clean, arch test green) · APP@1712870400.yNhgdh renamed "Model Registry" → "Module Catalog"; Module updated src/common/model_verifier_registry.py → src/domain/module_catalog.py; description rewritten to reflect immutable ModuleCatalog (Phase B). APP@1712870400.ca3vm7 (Model Verifier): Module updated src/common/model_verifier.py → src/application/verification/artifact_verifier.py; Dependencies updated to reflect port-based architecture (RuntimeCatalogs + 4 verifier ports). README Repository Layout: replaced stale src/common + src/tools with real six-package layout. mcp_artifact_server.py docstring: corrected Tool-logic-lives-in paths from src/tools/src/common to current src/infrastructure/mcp/... paths. Both entities verify clean. PLAN §6 Phase J1/J2 checkboxes ticked.
- 2026-06-09 · WU-16 · arch-conformance@1f9a844 · gates: all pass (1728 pytest, 1 skipped, ruff clean, zuban clean, arch test green) · Added check_module_source_path() to _verifier_rules_schema.py: warns W160 when Module: property references a non-existent source file; silently skips when no src/ ancestor found (repo not co-located with source). Wired into verify_entity_file(). 8 unit tests in test_verifier_module_source_path.py. source_file_length.py baseline updated: artifact_verifier.py 386→388. PLAN §6 Phase J3/J4 checkboxes ticked; §8 DoD "self-model + README" checkbox ticked.
