"""Protocol compliance tests for all registered OntologyModule and DiagramKindModule instances.
...
class TestOntologyModuleProtocol:
    def test_all_ontologies_are_isinstance(self, registry) -> None:
        for name, module in registry.all_ontologies().items():
            assert isinstance(module, OntologyModule), (
                f"Ontology {name!r} does not satisfy OntologyModule protocol"
            )
        ...
        for name, module in registry.all_ontologies().items():
            for type_name in module.entity_types:
                assert isinstance(type_name, str), (
                    f"Ontology {name!r}: entity type key {type_name!r} is not a str"
                )
    ...
    def test_connection_type_names_are_strings(self, registry) -> None:
        for name, module in registry.all_ontologies().items():
            for type_name in module.connection_types:
                assert isinstance(type_name, str), (
                    f"Ontology {name!r}: connection type key {type_name!r} is not a str"
                )
            ...
            for rel in module.permitted_relationships._rules:
                for type_ref in (rel.source_type, rel.target_type):
                    assert type_ref in known_types, (
                        ...
                        f"unknown entity type {type_ref!r}"
                    )
    ...
    def test_entity_types_with_class_subset_of_entity_types(self, registry) -> None:
        ...
        for name, module in registry.all_ontologies().items():
            ...
            assert internal.issubset(all_types), (
                f"Ontology {name!r}: internal class types not a subset of entity_types"
            )


# ── DiagramKindModule compliance ──────────────────────────────────────────────

class TestDiagramKindModuleProtocol:
    def test_all_diagram_kinds_are_isinstance(self, registry) -> None:
        for name, module in registry.all_diagram_kinds().items():
            assert isinstance(module, DiagramKindModule), (
                f"DiagramKind {name!r} does not satisfy DiagramKindModule protocol"
            )

    def test_name_matches_registry_key(self, registry) -> None:
        for key, module in registry.all_diagram_kinds().items():
            assert module.name == key, (
                f"DiagramKind key {key!r} does not match module.name {module.name!r}"
            )
        ...
        for name, module in registry.all_diagram_kinds().items():
            for type_name in module.effective_entity_types():
                assert type_name in all_entity_types, (
                    f"DiagramKind {name!r}: effective entity type {type_name!r} "
                    f"not in registry"
                )
        ...
        for name, module in registry.all_diagram_kinds().items():
            for type_name in module.effective_connection_types():
                assert type_name in all_conn_types, (
                    f"DiagramKind {name!r}: effective connection type {type_name!r} "
                    f"not in registry"
                )
        ...
        for name, module in registry.all_diagram_kinds().items():
            ...
            for t in effective:
                assert module.accepts_entity_type(EntityTypeName(t)), (
                    f"DiagramKind {name!r}: accepts_entity_type({t!r}) returns False "
        ...
        for name, module in registry.all_diagram_kinds().items():
            ...
            for t in effective:
                assert module.accepts_connection_type(ConnectionTypeName(t)), (
                    f"DiagramKind {name!r}: accepts_connection_type({t!r}) returns False "
