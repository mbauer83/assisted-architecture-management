...
listing (a finding is hidden when its component is) and the run-model aggregate."""
...
from src.application.security_refresh.signals_read import (
...
class _FakeStore:
    def __init__(self, runs: list[dict], components: dict[str, list], findings: dict[str, list]) -> None:
        self._runs = runs
    ...
    def get_active_run(self, anchor_entity_id: str) -> dict[str, Any] | None:
        return next(
            (r for r in self._runs if r["anchor_entity_id"] == anchor_entity_id and r["status"] == "active"),
            None,
        )

    def list_run_components(self, run_id: str) -> list[dict[str, Any]]:
        return list(self._components.get(run_id, []))
    ...
    def list_runs(self, *, anchor_entity_id: str | None = None) -> list[dict[str, Any]]:
        return [r for r in self._runs if anchor_entity_id is None or r["anchor_entity_id"] == anchor_entity_id]


def _store() -> _FakeStore:
    return _FakeStore(
        runs=[
            {"run_id": "R1", "anchor_entity_id": "APP@1", "status": "active"},
            {"run_id": "R0", "anchor_entity_id": "APP@1", "status": "superseded"},
        ],
        components={"R1": [
...
class TestComponents:
    def test_active_components_exposure_filtered(self) -> None:
        visible, withheld = list_active_components("APP@1", run_store=_store(), policy=_AMBER)
        assert [c["component_id"] for c in visible] == ["C1"]  # C2 is TLP:RED
        assert withheld == 1

    def test_no_active_run_is_empty(self) -> None:
        visible, withheld = list_active_components("APP@absent", run_store=_store(), policy=_AMBER)
...
class TestFindings:
    def test_finding_on_hidden_component_is_hidden_with_it(self) -> None:
        ...
        findings, withheld = list_active_findings("APP@1", run_store=_store(), policy=_AMBER)
    ...
    def test_scope_to_one_component_by_purl(self) -> None:
        findings, _ = list_active_findings(
            "APP@1", run_store=_store(), policy=_AMBER, purl="pkg:pypi/requests@2")
        assert [f["finding_id"] for f in findings] == ["F1"]

    def test_scope_to_absent_component_is_empty(self) -> None:
        findings, _ = list_active_findings(
            "APP@1", run_store=_store(), policy=_AMBER, component_id="C-absent")
        assert findings == []


class TestStats:
    def test_aggregate_counts_over_active_runs(self) -> None:
        stats = signals_stats(run_store=_store())
        assert stats["total_runs"] == 2
        assert stats["active_runs"] == 1
        assert stats["anchors_with_active_run"] == 1
        assert stats["active_run_components"] == 2  # unfiltered operational count
        assert stats["active_run_findings"] == 2
