"""SignalSnapshotToken (F3.15): a batch pinned to one snapshot goes
unavailable/retry — never partial — when the run activates, the ceiling
...
from src.application.security_refresh.snapshot import snapshot_still_valid, take_snapshot
...
def env(tmp_path: Path):
    ...
    yield {
        "store": store,
        "runs": SQLCipherRefreshRunStore(store._thread_conn_or_none),  # noqa: SLF001
        "vex": SQLCipherVexAssessmentStore(store._thread_conn_or_none),  # noqa: SLF001
    }
    store.lock()


def _refresh(env: dict[str, Any], anchor: str = "APP@1") -> None:
...
def _take(env: dict[str, Any], ceiling: str = "TLP:RED") -> Any:
    return take_snapshot(
        "APP@1", availability=env["store"], run_store=env["runs"],
...
def _valid(env: dict[str, Any], token: Any, ceiling: str = "TLP:RED") -> bool:
    return snapshot_still_valid(
        token, availability=env["store"], run_store=env["runs"],
        vex_store=env["vex"], exposure_ceiling=ceiling,
    )
...
class TestSnapshotInvalidation:
    ...
    def test_new_activation_invalidates(self, env: dict[str, Any]) -> None:
        _refresh(env)
        token = _take(env)
        _refresh(env)  # supersedes + activates a new run
    ...
    def test_first_run_appearing_invalidates_a_no_run_snapshot(self, env: dict[str, Any]) -> None:
        token = _take(env)
        assert token.active_run_id is None
        _refresh(env)
        assert not _valid(env, token)
