...
class TestRunImportHappyPath:
    def test_writes_cache_and_sidecar(self, source_file: Path, _isolated_cache_dir: Path) -> None:
        ...
        assert summaries[0].matched_keys == ("entity_types.stakeholder", "domain.motivation")
        assert summaries[0].unmatched_keys == ("entity_types.not-a-real-type", "domain.not-a-real-domain")
        ...
        assert a4["entity_types"]["stakeholder"]["create_when"] == "TEST-ONLY create_when text for stakeholder"
        assert "not-a-real-type" not in a4.get("entity_types", {})
        # v2 broader-level context: the valid domain node is kept, the unknown one dropped.
        assert a4["domain"]["motivation"]["context"] == "TEST-ONLY domain context for motivation"
        assert "not-a-real-domain" not in a4.get("domain", {})
...
class TestProvenanceSidecar:
    def test_sidecar_content(self, source_file: Path, _isolated_cache_dir: Path) -> None:
        ...
        assert sidecar["guidance_format"] == 2
        assert sidecar["matched_count"] == 2
        assert sidecar["unmatched_count"] == 2
        assert sidecar["unmatched_keys"] == ["entity_types.not-a-real-type", "domain.not-a-real-domain"]
